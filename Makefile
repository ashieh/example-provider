...
can_i_deploy: .env
	cd pact/bin; ./pact-broker can-i-deploy --pacticipant=${PACTICIPANT} --version=${TRAVIS_COMMIT} --to=prod --broker-base-url=${PACT_BROKER_BASE_URL} --broker-token=EKIaetf6uPElxwbO5b5KYA

deploy_app:
	@echo "Deploying to prod"

tag_as_prod:
	cd pact/bin; ./pact-broker create-version-tag \
...
create_or_update_pact_changed_webhook:
	cd pact/bin; ./pact-broker create-or-update-webhook \
